		...
		<div class="colbox">
			...
			<div class="clearfix info-showbox">
				...
				<div class="searchbox info-searchbox">
					<input id="jSearchItemInput" class="input-search" type="text" value="请输入您要搜索的物品名">
					<span class="publicspr tnd btn-search">搜索</span>
				</div>
				<div id="seleteCheckItemSec" class="colbox selete-seccheck" style="display:none">
					<!-- <span class="commspr commico-corner sbg"></span>
					<ul class="selete-checklist">
						<li class="selete-item current"><span class="ms-radio"><i class="i"></i></span><label data-item="">生命值</label></li>
					</ul> -->
				</div>
				<ul id="jSearchItemDiv" class="imgtextlist">
					
				</ul><!-- <li><img src="http://ossweb-img.qq.com/images/lol/web201310/101/tmp/tmp-user.jpg" alt=""><p>稻草人</p><span class="sbg"><i class="commspr commico-search"></i></span></li> -->
			</div>
			<!-- end 英雄列表 -->
		</div>
...
<script src="http://localhost:8080/Stream/js/common.js"></script>
<script>
//物品
var itemtree = {
	"consumable":"消耗品",
...
// doLoadHero = function(item, keyword){
// 	var retHTML= '',
// 	mark = 0,
// 	thisid = item.id,
// 	hrefs = "info-defail.shtml?id=",
// 	url = gets.u+item.image.group+'/'+item.image.full;
// 	if(thisid.toLowerCase().indexOf(keyword.toLowerCase())!='-1'|| item.name.indexOf(keyword)!='-1' || item.title.indexOf(keyword)!='-1' || item.key==keyword){
// 		retHTML+='<li><img src="http://ossweb-img.qq.com/images/lol/web201310/101/tmp/tmp-user.jpg" alt=""><p>稻草人</p><span class="sbg"><i class="commspr commico-search"></i></span></li>';
// 	}else{
// 		for(var x in item.tags){
// 			if(item.tags[x]==keyword){
// 				retHTML+='<li><img src="http://ossweb-img.qq.com/images/lol/web201310/101/tmp/tmp-user.jpg" alt=""><p>稻草人</p><span class="sbg"><i class="commspr commico-search"></i></span></li>';
// 			}
// 		}
// 	}
// 	return retHTML;
// },
getItemTree = function(keyword){
	var tree = LOLitemjs.tree,
 	strhtml = '',
 	str='',
 	classname='',
 	tagcont;
 	//加载查询子条件
 	for (var i in tree) {
 		if(keyword.toLowerCase()==tree[i].header.toLowerCase()&&"consumable"!=keyword.toLowerCase()){
 			for(var x in tree[i].tags){
	 			tagcont = tree[i].tags;
	 			str = tagcont[x].toLowerCase();
	 			strhtml+='<li data-item="'+str+'" class="selete-item"><span class="ms-radio"><i class="i"></i></span><label>'+itemtree[str]+'</label></li>';
	 		}
 		};
 	};
 	return strhtml;
},
doLoadItembyKeyword =function(keyword){
	var data = LOLitemjs.data,
	retHTML='';
	for (var x in data) {
		var itemHTML = doLoadItem(data[x], keyword);//模糊查询描述或者名字
		if(''!=itemHTML){
			retHTML+= doLoadItem(data[x],keyword);
		}
	};
	if(retHTML==''){retHTML = '<li class="loading">非常抱歉，查无此物品嗟！ 建议您输入正确的物品名称</li>'};
	g("jSearchItemDiv").innerHTML = retHTML;
},
doLoadItembyHeader = function(keyword){
	var data = LOLitemjs.data,
	tree = LOLitemjs.tree,     //获取物品的属性
	treetag = [],                    
	tmptag = '',
	str=null,
	retHTML='';
	//遍历出所有符合keyword的属性 放在treetag里
	for(var j in tree){
		if(keyword.toLowerCase()==tree[j].header.toLowerCase()){
			treetag=tree[j].tags;
			break;
		}
	};
	if(treetag.length==0){treetag[0]=keyword}
	console.log(treetag);
	for(var k=0,len=treetag.length;k<len;k++){
		str = treetag[k].toLowerCase();
		for (var x in data) {
			if(tmptag.indexOf(x)!='-1'){
				continue;
			}
			var itemHTML = doLoadItem(data[x], str);
			if(''!=itemHTML){
				tmptag+=x+',';
				retHTML+= doLoadItem(data[x],str);
			}
		};
	};
	g("jSearchItemDiv").innerHTML = retHTML;
},
doLoadItembySecNav = function(){
	if(gets.tag("seleteCheckItemSec","li")!=null){
		var sec = gets.tag("seleteCheckItemSec","li"),
		ord=0;
		//获取所有的选择框 
		for (var i = 0; i < sec.length; i++){
			...
			sec[i].onclick = function(){
				sec[ord].className = "selete-item";   //上一个被点的恢复原样式
				this.className = "selete-item current";//被点了自己样式 转化为被选中
				tags = sec[this.count].getAttribute("data-item");       //获取选择框上面的属性值
				doLoadItembyKeyword(tags);                         //把属性值筛选
				showPopup("jSearchItemDiv","li");                   //浮层
				ord = this.count;                                        //使用完把本次的作为旧的
			}
		}
...
swrapNAV = function(){
	var sw = gets.tag("seleteCheckItem","li"),
	secDIV = g("seleteCheckItemSec"),
	secHtml = '',
	ord=0,
	tags = sw[ord].getAttribute("data-item");
	doLoadItembyKeyword(tags);
	showPopup("jSearchItemDiv","li");
	checkInput("jSearchItemInput");
	g("jSearchItemInput").onkeyup = function(){
		doLoadItembyKeyword(this.value);
		showPopup("jSearchItemDiv","li");
	...
	for (var i = 0; i < sw.length; i++) {
		...
		sw[i].onclick = function(){
			...
			tags = sw[this.count].getAttribute("data-item");
			secHtml = getItemTree(tags);
			if(secHtml==''){
				secDIV.style.display="none";
			}else{
				secDIV.style.display="block";
				secDIV.innerHTML = '<span class="commspr commico-corner sbg sbgmove'+this.count+'"></span><ul class="selete-checklist">'+secHtml+'</ul>';
			}
			doLoadItembyHeader(tags);
			doLoadItembySecNav();
			showPopup("jSearchItemDiv","li");
			ord = this.count;
		}
...
loadScript("http://localhost:8080/Stream//js/item.js",swrapNAV);
</script>
</body>
</html><!--[if !IE]>|xGv00|5239d6eaff3d715315fcdd591e05848f<![endif]-->